...
class SubScreenManager(
    ...
    private val mCommandQueue: CmdQueue, displayId: Int
) {
    ...
    private var mStatusbar: SubStatusBarView? = null
    private var mNavibar: SubNavigationBarView? = null
    private val TAG = "SubScreenManager"
    ...
    fun initSubStatusBar() {
        Plog.i(TAG, "init-sub-StatusBar")
        ...
        mStatusBarParams.gravity = Gravity.TOP or Gravity.END
        mStatusbar =
            SubStatusBarView(mDisplayContext, mCommandQueue, mDisplayId)
    ...
    fun initSubNavigationBar() {
        Plog.i(TAG, "init-sub-NavigationBar")
        ...
        mNavibar = SubNavigationBarView(mDisplayContext, mCommandQueue, mDisplayId)
    ...
    fun updateSubStatusBar(focusable: Boolean, requestFocus:Boolean) {
        val params = mStatusbar?.getRootView()?.layoutParams as WindowManager.LayoutParams
        val flagToNotFocusable = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
        val flagSt = params.flags and flagToNotFocusable

        Plog.i(TAG, "updateSubStatusBar update-Sub-StatusBar focusable:$focusable " +
        ...
        runCatching {
            if (focusable) {
                Plog.i(TAG, "updateSubStatusBar requestFocus")
                params.flags = params.flags and WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE.inv()
                mDisplayWindowManager.updateViewLayout(mStatusbar?.getRootView(), params)
                mStatusbar?.getAvatarView()?.isFocusable = true
                if (requestFocus) {
                ...
                Plog.i(TAG, "updateSubStatusBar clearFocus")
                params.flags = params.flags or WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
                mDisplayWindowManager.updateViewLayout(mStatusbar?.getRootView(), params)
                mStatusbar?.getAvatarView()?.isFocusable = false
                if (requestFocus) {
                    GlobalDisplayFocusIndicatorManager.sendMessenger(
                        ScreenConfig.SECOND_DISPLAY_ID,
                        MSG_REQUEST_FOCUS,
    ...
    fun updateSubNavigationBar(focusable: Boolean, requestFocus:Boolean) {
        val params = mNavibar?.getRootView()?.layoutParams as WindowManager.LayoutParams
        val flagToNotFocusable = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
        val flagSt = params.flags and flagToNotFocusable

        Plog.i(TAG, "updateSubNavigationBar update-Sub-NavigationBar focusable:$focusable " +
                " requestFocus:$requestFocus flagSt:$flagSt")
        ...
        runCatching {
            if (focusable) {
                Plog.i(TAG, "updateSubNavigationBar requestFocus")
                params.flags = params.flags and WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE.inv()
                ...
                Plog.i(TAG, "updateSubNavigationBar clearFocus")
                params.flags = params.flags or WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
                mDisplayWindowManager.updateViewLayout(mNavibar?.getRootView(), params)
